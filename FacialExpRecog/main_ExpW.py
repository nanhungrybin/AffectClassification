...
# 1. expw
ExpW_train_dataset = CustomDataset(csv_file='/workspace/Affectnet/ExpW_annotation.csv', transform=transform, train = True )
ExpW_test_dataset = CustomDataset(csv_file='/workspace/Affectnet/ExpW_annotation.csv', transform=transform, train = False )
    
################################### 데이터 로더 생성 ###############################

ExpW_dataloader_train = DataLoader(ExpW_train_dataset, batch_size = config.batch_size, shuffle=True)
ExpW_dataloader_test = DataLoader(ExpW_test_dataset, batch_size = config.batch_size, shuffle=False)

###################################################################################

total_samples = len(ExpW_dataloader_train.dataset)
...
params = {
    ...
    "train_dataloader":ExpW_dataloader_train,
    "test_dataloader" : ExpW_dataloader_test,
    "device": device,
...
if __name__ == "__main__": # 객체를 불러 오는 것은 main함수에
    ...
    plt.ylabel('Actual')
    plt.title('ExpW - Confusion Matrix')
    plt.show()
